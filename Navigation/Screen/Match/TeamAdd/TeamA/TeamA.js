import {
  ...
  /* -----------------------refreshing ------------------------------*/
    ...
    React.useEffect(() => {
      console.log("Navigation/Screen/Match/TeamAdd/TeamA/TeamA.js");
      ...
      if(route.params?.TeamAName)
        setTeamAName(route.params?.TeamAName)

      if(route.params?.TeamAid)
        setSelectedItem(route.params?.TeamAid)
          ...
          const resposneJSON = await fetch(
            `${global.domainName}/cricbuddyAPI/api/CommonSp`,
            {
              ...
              headers: {
                ...
                "Content-Type": "application/json",
                Authorization: "FF7B5E5C-A468-4CE0-B812-98008627C8KT",
                SpName:"INDIVIDUALMATCH_API_CRUD"
              },
              body: JSON.stringify({
                OPER: Oper,
                SPNAME:"INDIVIDUALMATCH_API_CRUD",
                MOBILENO: global.MobileNo,
                TEAMAID:selectedItem,
                TEAMANAME:TeamAName,
                TEAMBID:"",
                TEAMBNAME:"",
                MATCHID:"",
                LASTPAGENAME:"TeamASelectPlayer"
              }),
            }
          )
            .then((response) => response.json())
            .then((json) => {
              ...
              if (BindData.SERVICERESPONSE.RESPONSECODE == "0") {
                ...
                if(BindData.SERVICERESPONSE.MATCHID)
                {
                  ...
                  if(BindData.SERVICERESPONSE.MATCHID)
                  {
                    var PageName = BindData.SERVICERESPONSE.TEAMANAME != '' ? BindData.SERVICERESPONSE.TEAMANAME : '';
