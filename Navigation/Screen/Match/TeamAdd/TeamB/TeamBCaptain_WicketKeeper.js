...
const TeamBCaptain_WicketKeeper = (props) => {
  ...
  React.useEffect(() => {
    console.log("Navigation/Screen/Match/TeamAdd/TeamB/TeamBCaptain_WicketKeeper.js");
    ...
    if (route.params?.TeamBid) setMyTeamId(route.params?.TeamBid);
    if (route.params?.TeamBName) setMyTeam(route.params?.TeamBName);
...
const BtnNext = async () => {
  ...
  if(selectedItem_Captain != null)
      ...
      try{
        const resposneJSON = await fetch(
          `${global.domainName}/cricbuddyAPI/api/TeamBCaptain_WicketKeeper`,
          {
            method: "POST",
            headers: {
              Accept: "application/json",
...
export default TeamBCaptain_WicketKeeper;