...
const MyMatch = (props) => {
  ...
  React.useEffect(() => {
    ...
    setListItems([]);
    console.log("Navigation/Screen/Match/MyMatch.js");
    if (route.params?.LoadRef == "True") {
      IndividualMatchList();
      // alert(global.MobileNo)
    }
  ...
  const IndividualMatchList = async () => {
    try {
      var data = {
        ...
        SPNAME: "INDIVIDUALMATCHLIST_GET",
      };
      ...
      const resposneJSON = await fetch(
        ...
        .then((json) => {
          ...
          if (BindData.SERVICERESPONSE.RESPONSECODE != "-1") {
            ...
            Alert.alert("Warning", "No Recored Found", [{ text: "OK" }]);
          }
          return json;
        })
        .catch((error) => {
  ...
  const btnModalOpen = async () => {
    ...
    if (MatchStart == "RESUMESCORING") {
      navigation.navigate(LastPageName, {
        ...
        Non_StickerPlayerid:
          LastPageName != "NextInning" ? Non_StickerPlayerid : null,
        Non_StickerName:
          LastPageName != "NextInning" ? Non_StickerName : "Non-Striker",
    ...
    <View style={styles.Container}>
        ...
        // >
          <View style={styles.BannerBox}>
            <TouchableOpacity
              onPress={() => {
                setTip(true);
                navigation.navigate("StartAMatch");
              }}
            >
              <View style={styles.BannerSpaceBetween}>
                <Text style={[styles.BannerTitle,]}>Want to start a match?</Text>
      ...
      {listItems.length == "0" ? (
        <View>
          ...
          <View style={[styles.body, { margin: 5 }]}>
            {/* <View
              style={[
                styles.body60,
                {
            ...
            <View style={[styles.body100, { padding: 5 }]}>
              <Custombutton
                title={"START MATCH"}
                onPress={() => navigation.navigate("StartAMatch")}
              />
            </View>
          </View>
        </View>
