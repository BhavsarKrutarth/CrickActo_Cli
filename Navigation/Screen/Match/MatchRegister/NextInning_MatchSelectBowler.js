import {
  ...
  // import { color } from "react-native-reanimated";
    ...
    /* -----------------------refreshing ------------------------------*/
    const NextInning_MatchSelectBowler = (props) => {
      ...
      React.useEffect(() => {
      console.log("Navigation/Screen/Match/MatchRegister/NextInning_MatchSelectBowler.js");
      
      if(route.params?.Matchid)
      {
        setMatchid(route.params?.Matchid)
        Bowler_GET(route.params?.Matchid)
      }
     ...
     /* -----------------------refreshing ------------------------------*/
      const Bowler_GET = async (Matchid) => {
        try {
            var data = {
                MATCHID:Matchid,
                WHERE_NQ_STICKERID:"",
                SPNAME:"MATCHTEAMABOWLERPLAYER_API_GET"
            }
            ...
            const resposneJSON = await fetch(
              `${global.domainName}/cricbuddyAPI/api/CommonSp`,
              {
                ...
                headers: {
                  ...
                  "Content-Type": "application/json",
                  Authorization: "FF7B5E5C-A468-4CE0-B812-98008627C8KT",
                  SpName:"MATCH_TEAMB_BOWLINGSTYLE_API_CRUD"
                },
                body: JSON.stringify({
                  OPER: 'Edit',
    ...
    export default NextInning_MatchSelectBowler;
