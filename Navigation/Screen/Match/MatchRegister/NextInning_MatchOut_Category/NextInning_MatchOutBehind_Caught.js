...
const NextInning_MatchOutBehind_Caught = () => {
  ...
  const [FielderName, setFielderName] = useState("Wicket-Keeper / Fielder");
  ...
  React.useEffect(() => {
    ...
    console.log("Navigation/Screen/Match/MatchRegister/NextInning_MatchOut_Category/NextInning_MatchOutBehind_Caught.js");
    console.log(route.params?.PosstionFlag + " || PosstionFlag")
  ...
  const FielderUpload = async () => {
    navigation.navigate("NextInning_MatchOut_FielderList", {
      ...
      ReturnPage: "NextInning_MatchOutBehind_Caught"
    });
  }
  const BtnNext = async () => {
  ...
  return (
    <View style={[styles.container]}>
      ...
      <Pressable style={styles.StrickerWrapper} onPress={() => FielderUpload()}>
        <Text style={styles.myText}>Wicket-Keeper</Text>
        <Image
          source={{
...
export default NextInning_MatchOutBehind_Caught;