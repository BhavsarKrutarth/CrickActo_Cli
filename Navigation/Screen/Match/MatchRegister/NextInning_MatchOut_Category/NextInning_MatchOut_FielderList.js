...
const NextInning_MatchOut_FielderList = () => {
    ...
    React.useEffect(() => {
        ...
        if (route.params?.Matchid) {
          ...
          Stiker_GET(route.params?.TeamABowlerid,route.params?.Matchid)
        }
        if (route.params?.ReturnPage) setReturnPage(route.params?.ReturnPage);
    });
    
    const Stiker_GET = async (TeamABowlerid,Matchid) => {
      
        try {
            var data = {
                WHERE_EQ_MYTEAMID: TeamABowlerid,
                WHERE_EQ_MATCHID: Matchid,
                SPNAME:"NEXTINNING_MATCHTEAMAPLAYER_GET"
              }
    ...
    const BtnNext = () => {
      if(ReturnPage == "NextInning_MatchOut_RunOut")
      {
...
export default NextInning_MatchOut_FielderList;