...
/* -----------------------refreshing ------------------------------*/
const NextInning_MatchNextBatterTeamB = (props) => {
  ...
  React.useEffect(() => {
    console.log("Navigation/Screen/Match/MatchRegister/NextInning_MatchOut_Category/NextInning_MatchNextBatterTeamB.js");

    if (route.params.Run) setRun(route.params.Run);
    if (route.params.BowlerRun) setBowlerRun(route.params.BowlerRun);
    if (route.params.ModalStickerSelect) setModalStickerSelect(route.params.ModalStickerSelect);
    ...
    if (route.params.Ball) setBall(route.params.Ball);
    ...
    if (route.params.RunDisplay) setRunDisplay(route.params.RunDisplay);
    if (route.params.NextOver) setNextOver(route.params.NextOver);
    if (route.params.NextInning) setNextInning(route.params.NextInning);
    if (route.params.Out) setOut(route.params.Out);
    if (route.params.Non_Stickerid) setNon_Stickerid(route.params.Non_Stickerid);
  ...
  const onRefresh = React.useCallback(() => {
    if (Matchid) {
      Stiker_GET(Matchid, Non_Stickerid);
    }
  ...
  const Stiker_GET = async (Matchid, Non_Stickerid) => {
    ...
    try {
      var data = {
        ...
        SPNAME: "NEXTINNING_MATCHTEAMBPLAYER_API_GET"
      }
        ...
        const resposneJSON = await fetch(
          `${global.domainName}/cricbuddyAPI/api/CommonSp `,
          {
            ...
            headers: {
              ...
              "Content-Type": "application/json",
              Authorization: "FF7B5E5C-A468-4CE0-B812-98008627C8KT",
              SpName: "NEXTINNING_MATCH_STRIKER_BATTINGSTYLE_API_CRUD"
            },
            body: JSON.stringify({
              OPER: 'Edit',
                ...
                <View style={styles.body100}>
                  ...
                  <View style={[styles.body100, styles.modal_imgcenter]}>
                    <Text style={styles.modalsubtitle}>
                      What's the style of{" "}
                      <Text style={{ fontWeight: "800",color:Color.FontColor }}>{Name}</Text> ?
                    </Text>
                  </View>
                </View>
                <View
...
const styles = StyleSheet.create({
  ...
  modaltitle: {
    fontSize: 20, color: Color.PrimaryColor, fontWeight: "600"
...
export default NextInning_MatchNextBatterTeamB;
