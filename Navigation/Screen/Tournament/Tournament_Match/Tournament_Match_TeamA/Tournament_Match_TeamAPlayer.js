...
/* -----------------------refreshing ------------------------------*/
const Tournament_Match_TeamAPlayer = (props) => {
  ...
  React.useEffect(() => {
    console.log(
      "Navigation/Screen/Tournament/Tournament_Match/Tournament_Match_TeamA/Tournament_Match_TeamAPlayer.js"
    );
    ...
    if (route.params?.MyTeam_GuId) setMyTeam_GuId(route.params?.MyTeam_GuId);
    ...
    if (route.params?.TeamAid) {
      ...
      Player_GET(
        route.params?.MyTeam_GuId,
        route.params?.TeamAid,
    ...
    if (route.params?.TeamAName) setMyTeam(route.params?.TeamAName);
    ...
    // navigation.addListener("beforeRemove", Tournament_Match_TeamAPlayer_onBeforeRemove); // listener added
    // return function cleanup() {
    //   navigation.removeListener("beforeRemove", Tournament_Match_TeamAPlayer_onBeforeRemove); // clean up
    // };
  ...
  // function Tournament_Match_TeamAPlayer_onBeforeRemove(event) {
  //   console.log("Tournament_Match_TeamAPlayer")
  ...
  const onRefresh = React.useCallback(() => {
    if (MyTeam_GuId) {
      Player_GET(MyTeam_GuId, MyTeamId, MyTeam, Tournament_Matchid);
      setRefreshing(true);
      setselectedSquad(0);
      setbtnDisplay(false);
    }
  ...
  }, [MyTeam_GuId, MyTeamId, MyTeam, Tournament_Matchid]);
  /* -----------------------refreshing ------------------------------*/
        ...
        .then((json) => {
          ...
          if (BindData.SERVICERESPONSE.RESPONSECODE != "-1") {
            if (BindData.SERVICERESPONSE.TOTALRECORDS != "0") {
              ...
              Player_GET(
                MyTeam_GuId,
                BindData.SERVICERESPONSE.MyTeamid,
                BindData.SERVICERESPONSE.MyTeam,
                Tournament_Matchid
              );
            } else {
  ...
  const Player_GET = async (
    ...
    try {
      ...
      if (TeamAid) {
        data.MyTeamid = TeamAid;
        data.MyTeam = TeamAName;
      }
      const resposneJSON = await fetch(
        `${global.domainName}/cricbuddyAPI/api/CommonSp`,
        {
        ...
        .then((json) => {
          ...
          if (BindData.SERVICERESPONSE.RESPONSECODE != "-1") {
            if (BindData.SERVICERESPONSE.TOTALRECORDS != "0") {
              ...
              setselectedSquad(
                setarray.filter((List) => List.Color === "green").length
              );
            } else {
              //setDisplayList("false")
            }
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={[styles.item, { borderColor: item.Color, borderWidth: 4 }]}>
  ...
  const BtnNext = async () => {
    ...
    if (listItems.filter((List) => List.Color === "green").length >= 2) {
      if (listItems.filter((List) => List.Color === "green").length <= 11) {
        try {
          const resposneJSON = await fetch(
            `${global.domainName}/cricbuddyAPI/api/Tournament_TeamASelectPlayer`,
            {
              method: "POST",
              headers: {
                Accept: "application/json",
              ...
              body: JSON.stringify({
                ...
                LastPageName: "Tournament_Match_TeamACaptain_WicketKeeper",
            ...
            .then((json) => {
              ...
              var List;
              navigation.navigate(
                "Tournament_Match_TeamACaptain_WicketKeeper",
                {
                  MyTeam,
                  MyTeamId,
                  Tournament_Matchid: Tournament_Matchid,
                  PageName: "Captain,Keeper",
                  TeamAPlayerCount: listItems.filter(
        ...
        Alert.alert("Warning", "Max 11 Player Select in Squad.", [
          { text: "OK" },
        ]);
      }
    } else {
      Alert.alert("Warning", "At List Two Player Select in Squad.", [
        { text: "OK" },
      ]);
    }
  };
          ...
          <Pressable
            onPress={() =>
              navigation.navigate("PlayerAddViaPhoneNo", {
                PLayer_RedirectPage: "Tournament_Match_TeamAPlayer",
                MyTeam: MyTeam,
                MyTeamId: MyTeamId,
              })
            }
...
export default Tournament_Match_TeamAPlayer;
