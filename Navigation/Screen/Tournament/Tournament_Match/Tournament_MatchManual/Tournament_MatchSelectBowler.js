import {
    ...
    /* -----------------------refreshing ------------------------------*/
    const Tournament_MatchSelectBowler = (props) => {
      ...
      React.useEffect(() => {
      console.log("Navigation/Screen/Tournament/Tournament_Match/Tournament_MatchManual/Tournament_MatchSelectBowler.js");
      ...
      Bowler_GET(route.params?.Tournament_Matchid)
     ...
     /* -----------------------refreshing ------------------------------*/
      const Bowler_GET = async (Tournament_Matchid) => {
        try {
            var data = {
                TOURNAMENT_MATCHID : Tournament_Matchid,
                SPNAME:"TOURNAMENT_MATCHTEAMBPLAYER_API_GET"
            }
                ...
                if (BindData.SERVICERESPONSE.TOTALRECORDS != "0") {
                  ...
                  if (BindData.SERVICERESPONSE.TOTALRECORDS > 1) {
                    if (List) {
                      List.forEach((List) => {
                        setarray.push({
                          id: List.TOURNAMENT_MATCHTEAMBPLAYERID,
            ...
            const resposneJSON = await fetch(
              `${global.domainName}/cricbuddyAPI/api/CommonSp`,
              {
                ...
                headers: {
                  ...
                  "Content-Type": "application/json",
                  Authorization: "FF7B5E5C-A468-4CE0-B812-98008627C8KT",
                  SpName:"TOURNAMENT_MATCH_BOWLINGSTYLE_API_CRUD"
                },
                body: JSON.stringify({
                  OPER: 'Edit',
                  TOURNAMENT_MATCHTEAMBPLAYERID: id,
                  TOURNAMENT_MATCHID:Tournament_Matchid,
                  PLAYERID:Playerid,
    ...
    export default Tournament_MatchSelectBowler;
