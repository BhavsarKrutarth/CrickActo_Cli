import {
    ...
    /* -----------------------refreshing ------------------------------*/
    const Tournament_MatchSelectNon_Striker = (props) => {
      ...
      React.useEffect(() => {
      console.log("Navigation/Screen/Tournament/Tournament_Match/Tournament_MatchManual/Tournament_MatchSelectNon_Striker.js")
      ...
      Stiker_GET(route.params?.Tournament_Matchid,route.params?.Stickerid)
       ...
       const onRefresh = React.useCallback(() => {
        if(Tournament_Matchid)
        {
            Stiker_GET(Tournament_Matchid,Stickerid);
        }
  
          // }
          
         wait(2000).then(() => setRefreshing(false));
       }, [Tournament_Matchid,Stickerid]);
     /* -----------------------refreshing ------------------------------*/
      const Stiker_GET = async (Tournament_Matchid,Stickerid) => {
        var data ={ 
          ...
          >
            <Pressable
              onPress={() => {
                ...
                SelectedItem(item.id,item.Tournament_Matchid,item.Name,`/cricbuddyAPI/UploadFiles/UserProfile/${item.ImageName}`,item.Playerid);
                setbtnDisplay(true)
      ...
      const SelectedItem = (id,Tournament_Matchid,Name,Img,Playerid) => {
        setid(id)
        setTournament_Matchid(Tournament_Matchid)
        setName(Name)
        setImg(Img)
            ...
            const resposneJSON = await fetch(
              `${global.domainName}/cricbuddyAPI/api/CommonSP`,
              {
                ...
                headers: {
                  ...
                  "Content-Type": "application/json",
                  Authorization: "FF7B5E5C-A468-4CE0-B812-98008627C8KT",
                  SpName:"TOURNAMENT_MATCH_STRIKER_BATTINGSTYLE_API_CRUD"
                },
                body: JSON.stringify({
                  OPER: 'Edit',
                  TOURNAMENT_MATCHTEAMAPLAYERID: id,
                  TOURNAMENT_MATCHID:Tournament_Matchid,
                  PLAYERID:Playerid,
    ...
    const styles = StyleSheet.create({
      ...
      modaltitle: { fontSize: 20, color: Color.PrimaryColor, fontWeight: "600",color:Color.FontColor },
      modalsubtitle: { fontSize: 16,color:Color.FontColor },
  
    });
    
    export default Tournament_MatchSelectNon_Striker;
